...
def main():
    ...
    # first input model
    visible1 = Input(shape = (number_steps, ))
    dense1 = Dense(100, activation = 'relu')(visible1)

    # second input model
    visible2 = Input(shape = (number_steps, ))
    ...
    # fit model
    model.fit([x1_input, x2_input], y_output, epochs = 2000, verbose = 0)

    # demonstrate prediction
    x_input = array([[70, 75], [80, 85], [90, 95]])

    x1_input = x_input[:, 0].reshape((1, number_steps))
    x2_input = x_input[:, 1].reshape((1, number_steps))
